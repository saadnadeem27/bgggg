...
class _FreeStyleWidgetState extends State<_FreeStyleWidget> {
  ...
  final objectRemoveController = Get.put(ObjectRemoveController());
  ...
  void _handleHorizontalDragDown(Offset globalPosition) {
    ...
    if (settings.mode == FreeStyleMode.draw) {
      ...
      print('Total path : ${[_globalToLocal(globalPosition)].toList()}');
      ...
      print('values : ${_globalToLocal(globalPosition)}');
      setState(() {
        // eraseComplete = false;
        objectRemoveController.setIsPathClear(false);
      });
    } else {
      return;
    }
  ...
  // bool isClear() {
  //   if (path.isEmpty) {F
  //     objectRemoveController.isPathEmpty.value = true;
  //     return true;
  //   } else {
  //     objectRemoveController.isPathEmpty.value = false;
  //     return false;
  //   }
  // }
  ...
  void _handleHorizontalDragUpdate(Offset globalPosition) {
    ...
    print('this drawable : ${this.drawable}');
  }
  ...
  void _handleHorizontalDragUp() {
    ...
    drawable = null;
    // if (drawable == null) {
    //   PainterController.of(context).performedActions.clear();
    //   PainterController.of(context).clearDrawables();
    // }
  }
